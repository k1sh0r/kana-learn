...
export function Header({ rightAction }: HeaderProps = {}) {
  ...
  // Use theme and setTheme only after mount
  const theme = mounted ? themeHook.theme : "system";
  const setTheme = mounted ? themeHook.setTheme : () => {};
        ...
        <div className="lg:container flex h-16 items-center justify-between px-4">
          ...
          <div className="flex md:hidden items-center gap-2 ml-auto">
            <Select value={language} onValueChange={setLanguage}>
              ...
              <SelectContent>
                <SelectItem value="en" icon={<Globe className="w-4 h-4 mr-1" />}>English</SelectItem>
                <SelectItem value="ru" icon={<Globe className="w-4 h-4 mr-1" />}>Russian</SelectItem>
              </SelectContent>
            </Select>
            <button 
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <Select value={language} onValueChange={setLanguage}>
              <SelectTrigger className="h-9 px-2 rounded-[0.75rem] border border-input hover:bg-accent hover:text-accent-foreground text-sm font-medium outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 flex items-center gap-2 min-w-[80px] flex-shrink-0" aria-label="Select language">
                <Globe className="w-4 h-4 mr-1" />
                <SelectValue>{language === "en" ? "EN" : "RU"}</SelectValue>
              </SelectTrigger>
              <SelectContent>
